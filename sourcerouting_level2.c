...
control MyIngress(inout headers hdr,
    ...
    // --- 修复点：使用 .setValid() 代替 .isValid = 1 ---
    action insert_srcRoute_2hops(bit<15> p1, bit<15> p2, macAddr_t dstMac) {
        hdr.srcRoutes.push_front(2);
        
        // 第 1 跳
        hdr.srcRoutes[0].setValid(); // 修正
        hdr.srcRoutes[0].port    = p1;
        hdr.srcRoutes[0].bos     = 0;

        // 第 2 跳
        hdr.srcRoutes[1].setValid(); // 修正
    ...
    // --- 修复点：使用 .setValid() 代替 .isValid = 1 ---
    action insert_srcRoute_3hops(bit<15> p1, bit<15> p2, bit<15> p3, macAddr_t dstMac) {
        hdr.srcRoutes.push_front(3);
    ...
    table ipv4_src_route_tbl {
        key = {
            hdr.ipv4.dstAddr: lpm;
        }
        actions = {
            insert_srcRoute_2hops;
            insert_srcRoute_3hops;
            drop;
        }
        size = 1024;
        default_action = drop;
